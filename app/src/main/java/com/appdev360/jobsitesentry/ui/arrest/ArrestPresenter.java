 ...
 */
public class ArrestPresenter extends BasePresenter<ArrestMvpView> {
    ...
    @Inject
    public ArrestPresenter(@ApplicationContext Context context, DataManager dataManager) {
    ...
    @Override
    public void attachView(ArrestMvpView mvpView) {
    ...
    public void getArrestVideos() {
        ...
        mSubscription = mDataManager.getNetworkService().arrestVideos("bearer"+authToken)
    ...
    private void parseResponse(BaseModel resp) {
        ...
        getMvpView().showItems(resp.getData().getVideos());
    }
}
